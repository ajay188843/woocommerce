...
//        $sel_qry .= " AND id='" . $current_selected_value . "'";
        ...
        foreach ($all_sel_rows as $all_sel_data) {
            ...
            $all_fetch_attribute_value_array[trim($all_option_attribute)] = trim($all_fetch_option_attribute_value); //for function
        }
...
//        foreach ($option_attribute as $option_attribute_key => $option_attribute_v) {
//            $querydetails .= " AND m1.meta_value REGEXP '.*\"name\";s:[0-9]+:\"" . $option_attribute_v . "\".*'";
//        }
...
////            $querydetails .= " AND (m1.meta_key = '_product_attributes' AND m1.meta_value LIKE '%s','%" . trim($option_attribute_value) . "%')";
//            $querydetails .= " AND m1.meta_value REGEXP '[[:<:]]" . $option_attribute_value . "[[:>:]]'";
...
//        $products_result = $wpdb->get_results($querydetails);
        ...
        $products_result = $this->get_matched_product_based_on_attribute_value($fetch_oopt_att_val_n_arr, $fetch_option_attribute_value_arr, $all_fetch_attribute_value_array);
        //exit();
        $product_html = '';
...
//            foreach ($products_result as $prd_data) {
    ...
    public function get_matched_product_based_on_attribute_value($fetch_oopt_att_val_n_arr, $fetch_option_attribute_value_arr, $all_fetch_attribute_value_array) {
        global $wpdb;
        $querydetails = "";
        $querydetails .= "SELECT *";
        $querydetails .= " FROM {$wpdb->prefix}posts";
        ...
        $querydetails .= " AND {$wpdb->prefix}posts.post_status = 'publish'";
        $querydetails .= " AND (m1.meta_key = '_product_attributes')";
        foreach ($all_fetch_attribute_value_array as $option_attribute_value_key => $option_attribute_va_value) {
//            $querydetails .= " AND m1.meta_value LIKE '%" . $option_attribute_va_value . "%'";
            $querydetails .= " AND m1.meta_value REGEXP '[[:<:]]" . $option_attribute_va_value . "[[:>:]]'";
        }
        $querydetails .= " ORDER BY {$wpdb->prefix}posts.post_date";
        $querydetails .= " DESC";
        echo "\n" . $querydetails;
...
//            }
            ...
            echo '</pre>';
//            $rsort_fetch_att_val_arr = rsort($fetch_attribute_value_array);
//            echo '<pre>';
//            print_r($rsort_fetch_att_val_arr);
//            echo '</pre>';
...
//            sort($fetch_oopt_att_val_n_arr);
//            sort($custom_product_attributes_arr_arr);
...
//                }
                $u = 0;
                //foreach ($fin_value_explode as $f_v_e_key => $f_v_e_value) {
...
//                                if (strpos($cp_value, '|') !== false) {
//                                    $cp_value_explode = explode('|', trim($cp_value));
//                                } else {
//                                    $cp_value_explode = array(trim($cp_value));
//                                }
...
//                                    echo "\n";
                            if (trim($fin_value) == trim($cpk_1_value)) { //in_array(trim($fin_value), $cpk_value)
//                                        echo "\n" . " Loop Count " . $j . "  Value  " . $f_v_e_value . "  Prev Value " . $prev_value;
                                echo '<pre>';
                                ...
                                if (!empty($previousValue) && $previousValue != '' && isset($previousValue)) {
                                    foreach ($previousValue as $prev_key => $prev_value) {
                                        //echo "\n" . " " . $j . "  Fetch Value : " . $fin_value . "  Prev Value : " . $prev_value . "  Current Value : " . $cpk_v_value;
                        ...
                        //}
//                        $previousValue = null;
//                        foreach ($cpk_value as $cp_key => $cp_value) {
//                            if (in_array($cp_value, $fetch_oopt_att_val_n_arr)) {
//                                echo "\n" . 'Match';
//                                echo $cpk_key;
//                                if ($previousValue) {
//                                    echo $previousValue;
//                                }
//                                $previousValue = $cp_value;
//                            }
//                        }
//                        echo '<pre>';
//                        print_r($previousValue);
//                        echo '</pre>';
//                            if (in_array($value, $cpk_value)) {
//                                echo "\n" . 'Match';
//                                echo $cpk_key;
//                            }
//                            $already_matched[$cpk_v_key] = $cp_value;
//                            foreach ($fin_value_explode as $fn_key => $fn_value) {
//                                $j = 0;
//                                foreach ($cpk_value_explode as $cp_key => $cp_value) {
////                                    echo "\n" . "Key - " . $fin_key . " == " . $cpk_v_key . "\n";
//                                    echo "\n" . "Value - " . $fn_value . " == " . $cp_value . "\n";
//                                    if (trim($fin_key) == trim($cpk_v_key) && trim($fn_value) == trim($cp_value)) {
//                                        echo "\n" . 'Match';
//                                        echo $cpk_key;
//                                        $already_matched[$cpk_v_key] = $cp_value;
//                                    }
