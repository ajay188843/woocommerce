...
/*
 * remove the action of the title.
 */
remove_action('woocommerce_shop_loop_item_title', 'woocommerce_template_loop_product_title', 10);
/*
 * hook : woocommerce_before_shop_loop_item
 * function : show_title,
 * description : customize product title.
 */
add_action('woocommerce_before_shop_loop_item', 'show_title');
 ...
 * description : this function used for the pagination before product. 
 */
add_action('woocommerce_before_shop_loop', 'woocommerce_pagination', 10);

/*
 * hook : woocommerce_after_shop_loop,
 * function : woocommerce_pagintion,
 * description : this function used for the pagination after product.
 */
add_action('woocommerce_after_shop_loop', 'woocommerce_pagination', 10);

/*
 * hook : after_setup_theme,
 * function : set_the_image_gallary,
 * description : this function used for the image gallay setting.
 */
add_action('after_setup_theme', 'set_the_image_gallary', 20);
 ...
 * function : wcs_woo_remove_reviews_tab,
 * description : this hook and function used for remove tab of reviews.
 */
...
/*
 * remove action for product data tabs.
 */
 ...
 * description : used for the add description tab after add to cart button.
 */
...
/*
 * hook : wp_login,
 * function : wc_user_login,
 * parameter : $user_login[username], $user[user_info_array],
 * Description : used for create coupon when user login first time of day.
 */
add_action('wp_login', 'wc_user_login', 10, 2);

function wc_user_login($user_login, $user) {
    global $current_user;
    get_currentuserinfo();
    $coupon_code_name = $user->user_login; // Code
    $user_email = $user->user_email;
    $user_email = array($user_email);
    $date = date('d/m/Y');
    $user_id = $user->ID;
    $last_login_date = get_user_meta($user_id, 'last_login', true);
    if ($date === $last_login_date) {
        
    } else {
        update_user_meta($user_id, 'last_login', $date);
        // copuon code generate 
        $amount = '10'; // Amount
        $discount_type = 'fixed_cart'; // Type: fixed_cart, percent, fixed_product, percent_product
        $coupon = array(
            'post_title' => $coupon_code_name,
            'post_content' => '',
            'post_status' => 'publish',
            'post_author' => 1,
...
/*
 * hook : wp_logout,
 * function : logout_user_delete_coupon,
 * description : remove the coupon when user logout.
 */
add_action('wp_logout', 'logout_user_delete_coupon', 20);
 ...
 * description : this function used for the delete coupon after the checkout process.
 */
...
/*
 * hook : user_register,
 * function : create_coupon_user_registration,
 * description : create coupon when new registration,
 * parameter : $user_id of register user.
 */
add_action('user_register', 'create_coupon_user_registration');

function create_coupon_user_registration($user_id) {

    $userinfo = get_userdata($user_id);
    $coupon_code_name = $userinfo->user_login;
 ...
 * function : user_restriction_to_use_coupon,
 * description : user ristriction to use coupon
 * parameter : no parameter. 
 */
add_action('woocommerce_applied_coupon', 'user_restriction_to_use_coupon', 20);
...
function user_restriction_to_use_coupon() {
    ...
    foreach ($my_coupon as $code => $coupon) {
        $total_discount += get_post_meta($coupon->id, 'coupon_amount', TRUE);
        if ($coupon_code != $coupon->code) {
            WC()->cart->remove_coupon($code);
            wc_clear_notices();
            wc_add_notice(__('you can not use this coupon.', 'woocommerce'), 'error');
        }
    }
    ...
    if ($amount < 500) {
        ...
        wc_clear_notices();
        wc_add_notice(__('you can not use this coupon now.', 'woocommerce'), 'error');
    }
}

add_action('woocommerce_cart_updated', 'update_the_coupon_discount');

function update_the_coupon_discount() {
    global $woocommerce;
    global $current_user;
    get_currentuserinfo();
    $totalamount = $woocommerce->cart->total;
    $coupon_code = $current_user->user_login;
    $user_coupons = WC()->cart->get_coupons();
